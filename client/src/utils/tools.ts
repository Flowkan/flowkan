...
export const applyDragResult = (boardData: Board, result: DropResult): Board => {
    ...
    if (type === "column") {
        ...
        const sourceColIndex = boardData.lists.findIndex(c => c.id?.toString() === sInd);
        const destColIndex = boardData.lists.findIndex(c => c.id?.toString() === dInd);
