...
describe("Testing of endpoints for types accounts", () => {
  test("Return of endpoint correct for type account valid", () => {
    const res = getAccountEndpoint("BASIC");
    expect(res).toBe(
			`${USER_ENDPOINTS.PROFILE}?account=BASIC`,
		);
  });

  test("Throws an error if the account type is invalid", () => {
    // @ts-expect-error: type not valid
    expect(() => getAccountEndpoint("GUEST")).toThrow("Not valid account.");
  });

  test("Throws an error if the account type in lowercase", () => {
    // @ts-expect-error: not valid type in toLowerCase
    expect(() => getAccountEndpoint("basic")).toThrow("Not valid account.");
  });

  test("works correctly with all valid enum values", () => {
...
describe("API Endpoint Generators", () => {
  test("USER_ENDPOINTS constants return correct static URLs", () => {
  ...
  test("BOARD_ENDPOINTS.BY_ID returns correct URL", () => {
    const id = "123";
    expect(BOARD_ENDPOINTS.BY_ID(id)).toBe(`/api/v1/boards/${id}`);
  });

  test("CARD_ENDPOINT.BY_ID returns correct URL", () => {
    const id = "456";
    expect(CARD_ENDPOINT.BY_ID(id)).toBe(`/api/v1/cards/${id}`);
  });

  test("LIST_ENDPOINT.BY_ID returns correct URL", () => {
    const id = "789";
...
describe("USER_ENDPOINTS.BY_ID", () => {
  test("should return the correct dynamic URL for a given profile ID", () => {
    const id = "123";
  ...
  test("should return correct URL even with special characters", () => {
